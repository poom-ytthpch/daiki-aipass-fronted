...
export default async function Plan(){
 ...
 <div className="pastelGrid sectionGap"><div className="pastelCard peach"><span>Used</span><strong>{fmt(u?.totalTokens)}</strong><small>{fmt(u?.inputTokens)} in · {fmt(u?.outputTokens)} out</small></div><div className="pastelCard sky"><span>Limit</span><strong>{q?.mode==='unlimited'?'∞':fmt(q?.limit)}</strong><small>{q?.interval||'lifetime'}</small></div><div className="pastelCard mint"><span>Natural reset</span><strong>{q?.resetAt?new Date(q.resetAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):'—'}</strong><small>{q?.resetAt?new Date(q.resetAt).toLocaleString():'No scheduled reset'}</small></div><div className="pastelCard blush"><span>Reset credits</span><strong>{fmt(q?.resetCredits?.available)}</strong><small>{q?.resetCredits?.nextExpiry?`Earliest expiry ${new Date(q.resetCredits.nextExpiry).toLocaleString()}`:'No active credits'}</small></div></div>
