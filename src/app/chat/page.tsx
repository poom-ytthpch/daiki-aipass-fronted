...
export default function Chat(){
  ...
  const generateGuestAttachment=async(kind:'image'|'file')=>{const prompt=text.trim();if(!prompt||kind==='image'&&!guest||kind==='file'&&pending)return;setAttachMenu(false);setUploadError('');setGuestGenerating(kind);try{if(kind==='file'){const generated=await requestGeneratedFile(prompt,requestedFileGenerationFormat(prompt));setAttachments(x=>[...x,generated]);return}const r=await fetch('/api/guest/generate/image',{method:'POST',headers:{'content-type':'application/json',...guestDeviceHeaders()},body:JSON.stringify({prompt,name:'generated.png'})});const d=await r.json().catch(()=>({})) as {attachment?:Attachment;error?:unknown};if(!r.ok||!d.attachment){const raw=errorText(d.error)||'image generation unavailable';throw new Error(friendlyGuestError(raw))}const generated=d.attachment;setAttachments(x=>[...x,generated])}catch(e){setUploadError(e instanceof Error?e.message:String(e))}finally{setGuestGenerating('')}};
